...
public class BoardController {
	...
	public String toBoardList(@RequestParam(name="page", defaultValue="1") int page,
		...
		// 한 페이지에 몇 개 == 5개
		// 버튼 몇 개 == 5
		// 총 게시글 개수 == SELECT COUNT(*) FROM TB_SPRING_BOARD WHERE STATS = 'Y'
	...
	public ModelAndView newBoard(ModelAndView mv
		...
		// 첨부 파일의 존재 유무 
		// => 차이점 => MultipartFile타입의 filename필드 값으로 확인을 하겠다 
		...
		// 1. 권한있는 요청인가
		// 2. 값들이 유효성이 있는 값인가 
		// 3. 전달된 파일이 존재할 경우 => 파일명 수정 서버에 올리고 BoardDTO의 changeName 필드에 값을 대입 
